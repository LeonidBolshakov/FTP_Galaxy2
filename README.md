...
- Программа учитывает различия в доступности отдельных FTP-файлов через сети разных
  операторов связи. Для проблемных файлов поддерживается `stop_list`, а параметр
  CLI `--mode stop-list|no-list` позволяет включать или отключать его применение.
  Итоговый отчёт явно указывает, какие файлы были исключены из синхронизации.
