...
def test_mode_type_valid_options():
    """Убедитесь, что mode_type возвращает правильные значения ModeDiffPlan для допустимых вариантов."""
    ...
    # регистронезависимо и допускаются окружающие пробелы
    assert config_CLI.mode_type("  STOP-LIST  ") is ModeDiffPlan.USE_STOP_LIST


def test_mode_type_invalid_option():
    """mode_type должен вызывать исключение при недопустимом варианте."""
...
def test_parse_args_missing_config(monkeypatch):
    """Функция parse_args должна поднять ConfigError, если не указан путь к файлу конфигурации."""
